...
async function getAllTodoTasksByUserId(
...
async function insertTodoTask(fastify: FastifyInstance, todo: TodoDTO) {
  try {
    const [result] = await fastify.mysql.query<ResultSetHeader>(
      "INSERT INTO todos (user_id, title, content, task_level, expiration) VALUES (?, ?, ?, ?, ?)",
      [
        todo.userId,
        todo.title,
        todo.content ?? null,
...
async function updateTodoTask(
  ...
  try {
    const [result] = await fastify.mysql.query<ResultSetHeader>(
      "UPDATE todos SET title = ?, content = ?, task_level = ?, expiration = ? WHERE id = ?",
      [
        todo.title,
        todo.content ?? null,
        todo.taskLevel,
...
async function deleteTodoTask(
  ...
  try {
    ...
    const [todoRows] = await fastify.mysql.query<RowDataPacket[]>(
